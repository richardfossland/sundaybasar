...
beforeEach(() => {
  ...
  state.table = ''
  state.ops = []
  state.result = { data: [{ id: 's1' }], error: null }
...
describe('GET /api/health', () => {
  it('answers 200 without touching the database', async () => {
    ...
    expect(body.app).toBe('sundaybasar')
  ...
  it('?db=1 runs the lightest possible read and reports ok', async () => {
    ...
    expect(res.status).toBe(200)
    const body = (await res.json()) as Record<string, unknown>
    expect(body).toMatchObject({ ok: true, app: 'sundaybasar', db: 'ok' })
    expect(typeof body.ms).toBe('number')
    expect(state.table).toBe('sessions')
